...
export class ScriptGeneratorComponent {
  ...
  loading = {
    ...
    salesScript: false,
    solutions: false,
    objectionHandlers: false,
    closingStatements: false,
  };
  ...
  async handleResearch(e: Event) {
    ...
    try {
      const response = await fetch(HttpRequestsConstants.GENERATE_SCRIPT, {
        method: "POST",
        headers: {
  ...
  getPainPointDescription(painPointId: string): string {
    const painPoint = this.scriptData?.pain_points.find(
      (p) => p.id === painPointId
    );
